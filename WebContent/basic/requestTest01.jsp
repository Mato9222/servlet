...
<%--
	이것은 JSP 주석을 나타냅니다 - 소스보기에 나타나지않는다
--%>
...
<%--
	<%=수식이나 변수명 %> ==> JSP문서에서 변수나 수식의 결과를 출력할 때 사용한다.
					==> '표현식(Expresion)'이라고 한다.
 --%>

<!--  
	<form>태그의 속성
1) acrion => <form>에서 만들어진 데이터를 받아서 처리할 문서명 또는 서블릿 URL
2) method => 정송방식 (GET 또는 POST) ==> 기본값 : GET
3) target => 응답이 나타날 프레임 지정
4) enctype => 서버로 파일을 전송할 때 사용하는 속성으로 속성값은 
			'multipart/form-data'로 지정한다.
 -->

<h2> <%=name %>Request연습용 Form</h2>
