...
public class ForwardMain extends HttpServlet {
	...
	/*
	 - forward 방식
	 	1) 특정 서블릿이나 JSP에 대한 여청을 다른 서블릿이나 JSP로 넘겨준다.
		 	(이 때 HttpServletRequest객체와 HttpServletResponse객체를 공유하기때문에
		 	파라미터를 넘길 수 있다.)
	 ...
	 읽기 형식) Request객체.getAttribute("key값");
	 		- 'key값'은 문자열로 지정하고, '저장할 데이터'는 모든 데이터를 사용 가능 
	 */
		...
		RequestDispatcher rd = 
			request.getRequestDispatcher("/forwardTarget.do");
