...
/*
 	Servlet 동작 과정...(Servlet의 LifeCycle) 
 		1. 사용자(클라이언트)가 URL을 입력하거나 클릭하면 HTTP요청(Request)을 
 			Servlet Container로 전송(요청) 한다.
 		2. 컨테이너는 web.xml에 정의된 URL 패턴을 확인하여 어느 서블릿을 통해 처리할지를 검색한다.
 			(로딩이 안된 경우에는 해당 서블릿을 로딩한다. 처음 로딩시 init()메서드가 자동으로 호출된다.
 			(Servlet 3.0이상에는 애노테이션으로 설정이 가능하다.)
 		3. Servlet Container는 개별 요청을 처리할 쓰레드를 생성하여 해당 서블릿 객체의
 		 	service()메서드를 호출한다. (이 때 HttServletRequest객체와 HttpResponse객체를
 		 	생성하여 파라미터로 넘겨준다.)
 		4. service()메서드는 전송방식(GET, POST등)을 체크하여 적절한 메서드가 호출한다.
...
//		super.service(request, response);
		...
		// 방법2) 클라이언트의 전송방식(GET, POST등)을 직접 구분해서 직접 메서드 호출하기 
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<html><head><meta charset='utf-8'></head>"
				+ "<body><h1 style='color:red'>doGet()메서드의 처리결과입니다."
				+ "</h1></body></html>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<html><head><meta charset='utf-8'></head>"
				+ "<body><h1 style='color:blue'>doPost()메서드의 처리결과입니다."
				+ "</h1></body></html>");
	}
